...
async def diary_handler(message: Message):
    ...
    if not profile or not profile.get('consent'):
        await message.answer("📝 Сначала заполните анкету!", reply_markup=main_menu())
        return
...
(Файл отправляется сразу)
    ...
    try:
        ...
        await message.answer("📘 Файл будет доступен совсем скоро!")
...
async def yes_no_handler(callback: CallbackQuery, state: FSMContext):
    if callback.data == "yes":
        ...
        await callback.message.edit_text("❌ Вы отказались от обработки данных")
        await state.clear()
        await callback.answer()
